...
class PathSelector extends Component {
  ...
  render() {
    ...
    const getIcon = i => <i className="fa fa-stop push-right push-left" style={{ color: colors[i] }} />;
    ...
    const items = _.map(colors, (color, i) => (
      <MenuItem bsSize="small" eventKey={i} key={`path-select-${i}`}>
    ...
    return (
      <DropdownButton bsSize="small" title={getIcon(pathIdx)} id="pathSelector" onSelect={::this.onPathSelect}>
        {items}
      </DropdownButton>
    );
  }

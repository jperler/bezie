...
class ContextMenu extends Component {
  ...
  render() {
    ...
    const isEndpoint = utils.isEndpoint(path, selected) || utils.isPathEndpoint(path, selectedIdx);
    ...
    const getPointDisplay = type => {
      switch (type) {
        ...
        case pointTypes.cubic:
          return "Bezier 2";
        default:
          return isEndpoint ? "Endpoint" : "Default";
      }
    };
    ...
    return (
      <div className={styles.menu}>
        <DropdownButton
          ...
          disabled={isEndpoint}
        >
          <MenuItem bsSize="xsmall" disabled={isDefault || isEndpoint} eventKey="default">
            Default
          </MenuItem>
          <MenuItem bsSize="xsmall" disabled={!isDefault || isEndpoint} eventKey={pointTypes.quadratic}>
            {getPointDisplay(pointTypes.quadratic)}
          </MenuItem>
          <MenuItem bsSize="xsmall" disabled={!isDefault || isEndpoint} eventKey={pointTypes.cubic}>
            {getPointDisplay(pointTypes.cubic)}
          </MenuItem>
        </DropdownButton>
        {!isEndpoint && (
          <a href="javascript:void(0)" className="push-left" onClick={::this.onRemovePointClick}>
            Remove
          </a>
        )}
        <span className="push-left monospace noselect">{`[${formatted.x}, ${formatted.y}]`}</span>
      </div>
    );
  }
}

...
import { ControlLabel, InputGroup, Form, ButtonToolbar, Button, FormGroup, FormControl } from "react-bootstrap";
      ...
      <div>
        <div className="pull-left">
          {this.props.settings.midi.map((config, pathIdx) => (
            <Form inline key={pathIdx} className="push-bottom-xsmall">
              <FormGroup>
                <InputGroup>
                  <InputGroup.Addon>
                    <i className="fa fa-square" style={{ color: colors[pathIdx] }} />
                  </InputGroup.Addon>
                  <FormControl
                    type="text"
                    label="Text"
              ...
              <FormGroup className="push-left-xsmall">
                ...
                >
                  <option value={PITCH} disabled={_.includes(currentChannels, PITCH)}>
                    Pitch
                  </option>
                  {_.map(_.range(NUM_CC_CHANNELS), i => (
                    <option key={i + 1} value={i + 1} disabled={_.includes(currentChannels, i + 1)}>
                      Channel {i + 1}
                    </option>
                  ))}
                </FormControl>
              ...
              {mode === modes.controller && (
                <FormGroup className="push-left-xsmall">
                  <InputGroup>
                    ...
                    <FormControl type="text" disabled value={this.getMapLabel(pathIdx)} onChange={_.noop} />
                    <InputGroup.Addon>
                      <Button
                        style={{ fontSize: 11 }}
                        bsSize="xsmall"
          ...
          <ButtonToolbar className="push-top">
            <Button bsSize="small" onClick={::this.onSaveClick}>
              Update
            </Button>
            <Button bsSize="small" onClick={() => (window.location = "#/")}>
              Cancel
            </Button>
            <Button bsSize="small" onClick={::this.onSavePresetClick}>
              Save Preset
            </Button>
            <Button bsSize="small" onClick={::this.onLoadPresetClick}>
              Load Preset
            </Button>
          </ButtonToolbar>
        </div>
        <div className="pull-left push-left" style={{ width: 200 }}>
          <Form>
            <FormGroup>
              ...
              <FormControl componentClass="select" value={this.state.mode} onChange={::this.onModeChange}>
              ...
              <FormGroup>
                ...
                >
                  ...
                  {_.map(midiUtil.getControllers(), data => (
                    <option key={data.label} value={data.index}>
                      {data.label}
                    </option>
                  ))}
                </FormControl>
                <Button onClick={::this.onRefreshControllersClick} bsSize="small" className="push-top">
                  Refresh Controllers
                </Button>
              </FormGroup>
            )}
            {mode === modes.controller && (
              <FormGroup>
                ...
                <FormControl componentClass="select" value={this.state.tempo} onChange={::this.onTempoChange}>
